...
const controller = {
    async insertVechileinfo(req, res) {
        try {
            const { v_owner_mobile } = req.body.v_owner_mobile
            const { v_reg_number } = req.body.v_reg_number
            ...
            const insertInfo = async () => {
                const registerVechile = await userDetails.create({
                    v_type,
                    v_name,
                    v_ownername,
                    v_owner_mobile,
            ...
            if (!findMobilenum) {
                ...
                rres.status(403).json({ status: false, message: 'Mobile number already exist' });
            }
        } catch (error) {
